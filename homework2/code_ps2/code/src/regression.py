...
class PolynomialRegression() :
    ...
    def generate_polynomial_features(self, X) :
        ...
        Phi = np.array([map(lambda n: pow(x[0], n), range(0,m+1)) for x in Phi])
        ### ========== TODO : END ========== ###
...
def main() :
    ...
    print 'Coefficient and Cost by Closed-form...'
    model.fit(train_data.X, train_data.y)
    print model.coef_
    print model.cost(train_data.X, train_data.y)
    print 'Coefficient and Cost by GD...'
    model.fit_GD(train_data.X, train_data.y)
    print model.coef_
    print model.cost(train_data.X, train_data.y)
    
    
    phi = model.generate_polynomial_features(train_data.X)
    n,d =phi.shape
#    print phi
#    print train_data.X
...
#    print c
    ...
    for m in range(0,11):
        model = PolynomialRegression(m)
        model.fit(train_data.X, train_data.y)
        rmse_train[m]=model.rms_error(train_data.X, train_data.y)
        rmse_test[m]=model.rms_error(test_data.X, test_data.y)
        print rmse_test[m]
    plt.plot(rmse_train.keys(), rmse_train.values(), 'b--', label='training')
    plt.plot(rmse_test.keys(),rmse_test.values(), 'g--', label='testing')
